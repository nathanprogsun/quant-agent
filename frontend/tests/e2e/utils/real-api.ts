 ...
 * Get thread details
 */
export async function getThread(page: Page, threadId: string): Promise<{ id: string; title?: string }> {
  const response = await page.request.get(`${BACKEND_URL}/api/v1/threads/${threadId}`);
 ...
 * Update thread title
 */
export async function updateThreadTitle(page: Page, threadId: string, title: string): Promise<void> {
  const response = await page.request.patch(`${BACKEND_URL}/api/v1/threads/${threadId}`, {
    data: { title },
  });
...
export async function deleteThread(page: Page, threadId: string): Promise<void> {
  const response = await page.request.delete(`${BACKEND_URL}/api/v1/threads/${threadId}`);
 ...
 * List all threads
 */
export async function listThreads(page: Page): Promise<{ threads: Array<{ id: string }> }> {
