...
from app.skills.tool_policy import tool_name

logger = logging.getLogger(__name__)


class PromotedState(TypedDict, total=False):
    """Catalog-hash-scoped promotion record stored under ``state["promoted"]``."""

    catalog_hash: str
    names: list[str]


class DeferredToolFilterState(AgentState):
    """State read by :class:`DeferredToolFilterMiddleware`."""

    promoted: NotRequired[PromotedState]
...
class DeferredToolFilterMiddleware(AgentMiddleware[AgentState]):
    ...
    def _promoted(self, state: Any) -> set[str]:
    ...
    def _hidden(self, state: Any) -> set[str]:
