...
class AuthService:
    ...
    async def initialize_system(
        ...
        Raises ConflictResourceError if any user already exists.
        """
        user_count = await self.user_service.count_users()
        if user_count > 0:
            raise ConflictResourceError("System already initialized")
        hashed_password = self.get_password_hash(password)
