...
def _build_eval_retriever() -> JqApiRetriever:
    """Eval uses real LLM query-gen — inject explicitly, not lazy default."""
...
async def _run_eval(dataset: Path, *, k: int = 5) -> dict:
    ...
    for q in questions:
        ...
        expected = q.get("expected_function", "")
            ...
            failures.append(
                {
                    ...
                    "query": q["query"],
                    "expected": expected,
                    "got": [h.metadata.get("function_name") for h in hits],
                }
            )
...
def main() -> None:
    ...
    dataset = EVAL_DATA_DIR / ("jq_api_eval_pilot.jsonl" if args.pilot else "jq_api_eval_full.jsonl")
    if not dataset.is_file():
        raise FileNotFoundError(dataset)

    report = asyncio.run(_run_eval(dataset, k=args.k))
    EVAL_REPORT_DIR.mkdir(parents=True, exist_ok=True)
    out = EVAL_REPORT_DIR / f"retrieval_jq_api_{'pilot' if args.pilot else 'full'}.json"
